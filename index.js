...
console.log(
  "ACCESS_TOKEN_SECRET:",
  process.env.ACCESS_TOKEN_SECRET ? "FOUND" : "NOT FOUND"
);
...
app.get("/", (req, res) => {
  res.send({
    success: true,
    message: "Shop Jewellers Server Running",
  });
});
...
app.post("/sales", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const saleDoc = {
      ...
      quantity: qty, sellPrice: price, buyPrice: safe(product.buyPrice),
      revenue, cost, profit, createdAt: new Date()
    };
...
app.get("/dashboard", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    res.send({
      success: true,
      stats: {
        totalSales, totalCash, totalExpense, totalStock,
        totalStaff: staff.length, totalReceivables: receivables.length
      }
    });
  } catch (error) {
